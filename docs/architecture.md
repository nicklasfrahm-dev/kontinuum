...
`pkg/cli` holds the cobra command tree: `kontinuum serve` (start the server), `kontinuum version`, `kontinuum config` (import/inspect configuration), `kontinuum zone add` (fan out a new zone's `Zone`/`Instance`/`InstancePool`/`TalosCluster` objects — see [Add zone](workflows/zone-add.md)), and `kontinuum fabricmanager run` (the `fabric` controller's own per-zone network agent workload — see below). `pkg/config` loads configuration from `KONTINUUM_`-prefixed environment variables, with CLI flags overriding it when explicitly set.
...
| `fabric` | Carves a fixed-size subnet and gateway IP per zone out of a `Fabric`'s region-wide `spec.cidr` (deterministic, sticky IPAM — see `Allocate`), elects a per-zone gateway node via `spec.gatewaySelector`, and assigns that zone's own gateway address (via Talos) to every one of the node's own interfaces that isn't already carrying an address — its existing uplink is left alone, and is what a small privileged `kontinuum fabricmanager run` workload (nftables masquerade, over netlink — no `nft(8)` binary needed) then masquerades outbound traffic through. More than one free interface gets bridged together first (STP enabled as a loop-prevention default), since Linux can't assign the same address to several raw interfaces independently — `status.zones[].gatewayInterfaces` reports exactly which interfaces ended up carrying it. That node agent is named for its growing scope (DHCP and other per-zone network duties are expected to land in it later), even though NAT is all it does today. Each zone's own network is independent: one zone's missing gateway candidate (or one with only a single, already-addressed interface — a gateway needs at least one free interface beyond its own uplink) only blocks that zone's own readiness, never the whole `Fabric`'s. A finalizer tears down each zone's own agent workload (not the pushed Talos config) when a `Fabric` is deleted. |
