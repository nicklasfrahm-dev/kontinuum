...
| `taloscluster` | Bootstraps a Talos Kubernetes cluster from a control-plane `InstancePool` (and optional worker pools), then installs Cilium and cert-manager as addons. Once converged, keeps every member on the Talos/Kubernetes versions `spec` pins, rolling them one at a time — see [Upgrade a cluster](workflows/cluster-upgrade.md). State machine driven by `status.conditions`. |
