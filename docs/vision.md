...
| Underlay networking (VLANs, L2/L3 provisioning) | 🚧&nbsp;In&nbsp;progress | The `fabric` controller carves a per-zone subnet/gateway IP out of a region-wide `Fabric.spec.cidr` and installs a per-zone NAT gateway (nftables masquerade, elected node via `spec.gatewaySelector`) — [issue #124](https://github.com/nicklasfrahm-dev/kontinuum/issues/124). Not yet covered: VLAN assignment, DHCP for non-gateway nodes, and switch-side config — kontinuum still doesn't configure switches or physical network topology. |
