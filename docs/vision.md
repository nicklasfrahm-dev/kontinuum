...
| Pool claiming | 🚧&nbsp;In&nbsp;progress | `InstancePool` claims discovered `Instance`s up to `spec.replicas` via a conditional (CAS) update, so two pools can never claim the same machine — eligibility gating and on-demand provisioning are still evolving. See [Cluster provisioning](workflows/cluster-provisioning.md). |
